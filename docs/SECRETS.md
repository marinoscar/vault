...
| `cvv` | `string` | Yes | Yes | CVV/CVC — the card security code. Extracted by the AI card-scanning endpoint along with every other field, and encrypted at rest like any other field (see [Security Considerations](#security-considerations)) |
| `security_code_2` | `string` | No | Yes | An ADDITIONAL security code, for cards that print more than one. `null` on the ordinary card that prints only one (this now includes an ordinary American Express, whose single front-printed CID is `cvv`) — never a duplicate of `cvv` |
...
**The CVV is now extracted, at the repository owner's explicit direction — this reverses a previously documented structural exclusion.** `cvv` is a normal member of `EXTRACTED_FIELD_NAMES` (`apps/api/src/ai/providers/vision-provider.interface.ts`), has a key in the OpenAI response schema, and seeds the review form like every other field; the review form no longer forces it to an empty string. `normalizeSecurityCode` (`apps/api/src/ai/card-normalizer.ts`) accepts only a 3- or 4-digit result, nulling anything else with a warning, and nulls `security_code_2` whenever it duplicates `cvv`.

State the consequence plainly: **the PAN and the security code now sit together in the vault**, so a compromise of the vault (a stolen `VAULT_ENCRYPTION_KEY` plus database access, or a bug that leaks decrypted secret data) yields a complete card-not-present credential in one place, rather than a card number that still needs its CVV supplied from elsewhere. This is the same posture as mainstream consumer password managers, which routinely store both fields together, and it is a deliberate decision by the application owner, not an oversight or a regression to be fixed. It does not change what already left this application's boundary: the photograph handed to the extraction endpoint already contained the security code (it is printed on the card being photographed) and that photograph was already sent to OpenAI whether or not the model was asked to read the code back — extracting `cvv` adds no NEW third-party egress, it only decides whether the value already in the provider's view is also returned to this application. Once saved, the value is encrypted at rest with AES-256-GCM exactly like every other secret field, via the same `CryptoService` used throughout this document — storing it introduces no new encryption gap, only a larger blast radius if that encryption is ever defeated.

**Where this would stop being acceptable: PCI-DSS.** Retaining the card verification code after authorization is prohibited sensitive-authentication-data storage under PCI-DSS (Requirement 3.3.1 in v4.0, 3.2 in v3.2.1). That requirement binds entities that store, process, or transmit cardholder data as part of the payments ecosystem — merchants, processors, service providers — and a person keeping their own cards in their own password manager is not such an entity, which is why consumer password managers store this field as a matter of course. The distinction is the deployment, not the code: standing this application up as a multi-tenant service, using it to hold an organisation's corporate cards, or wiring it into anything that authorises transactions would very plausibly pull it into PCI-DSS scope, and at that point storing `cvv` becomes a compliance violation rather than a product decision. Anyone contemplating one of those deployments should treat removing this field — not merely re-reviewing it — as a prerequisite.

`security_code_2` no longer documents the American Express front CID — that value **is** `cvv` now (for American Express, `cvv` is the 4-digit code on the front, above the account number; for other networks it remains the 3-digit code on the back). `security_code_2` is redefined to mean an ADDITIONAL code, present only on cards that print more than one; it is `null` for the ordinary card, including an ordinary Amex, and the normalizer nulls it outright if the model ever reports it as a duplicate of `cvv`.

`EXTRACTED_FIELD_NAMES` also adds `notes`: auxiliary, non-sensitive printed text (customer service numbers, a "Member Since" year, a website, a contactless indicator, usage instructions) that the other fields have no slot for. Both the prompt and the response schema explicitly ban the PAN and any security code — `cvv` or `security_code_2` — from `notes`; on the wire it is truncated at 1000 characters rather than the 200 applied to other string fields. On card renewal, extracted `notes` are appended under the secret's existing notes rather than overwriting them (`apps/web/src/hooks/useCardRenewal.ts`); by contrast, the outgoing card's `cvv` is never carried forward into a renewal — only a code freshly read off the *new* card's photo seeds the field, since a reissued card always carries a new code (see `CVV_FIELD` in `useCardRenewal.ts`). See [API.md](API.md#ai-card-extraction) and [SECURITY-ARCHITECTURE.md](SECURITY-ARCHITECTURE.md) §8 (AI Features and Data Egress) for the full data-flow, key-storage, and mitigation details.
