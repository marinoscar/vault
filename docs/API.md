...
**The card security code (`cvv`) is extracted.** `cvv` is a normal member of the requested field set (`EXTRACTED_FIELD_NAMES`), has a key in the OpenAI response schema, and appears in both `fields` and `confidence` in the response below like every other field — see the worked example. This endpoint still persists nothing itself (see above); once the reviewed value is saved as a secret it is encrypted at rest exactly like every other secret field (AES-256-GCM) — see [SECRETS.md § Security Considerations](SECRETS.md#security-considerations) for the consequence of storing PAN and security code together.
...
Fields are `null` (with confidence `0`) only when nothing is visible for that field at all. This is a deliberate best-guess policy: the model is instructed to always return its best reading — including faint or low-contrast text on laser-engraved metal cards — with an honest, possibly low, confidence score and a `warnings` entry, rather than withholding a value it isn't fully sure of. Note the `4111 1111 1111 1111` test PAN used above is the well-known Luhn-valid test number; it is not a real card.

**`cvv` is normalized server-side to digits-only, and only accepted at a real length.** `normalizeSecurityCode` (`apps/api/src/ai/card-normalizer.ts`) strips non-digit characters and keeps the result only if it is exactly 3 or 4 digits — the two lengths a real security code can have (3 for Visa/Mastercard/Discover/most networks, 4 for American Express). Anything else (the model having read the last four PAN digits, an expiry, or a "MEMBER SINCE" year instead) is nulled with its confidence reset to `0` and a `warnings` entry ("The security code could not be read reliably — type it from the card."), the same treatment applied to every other field the model may misread. `security_code_2` goes through the identical 3-or-4-digit normalization and, in addition, is nulled (confidence `0`, no warning) whenever it comes back equal to `cvv` — a model that finds one code and reports it twice must not hand the reviewer a phantom "second code" to reconcile.

**`security_code_2` now means an *additional* code, not the Amex CID.** Before this change, `security_code_2` documented the 4-digit code printed on an American Express card's front (its CID) as a value distinct from `cvv`. That CID **is now `cvv` itself** — for American Express, `cvv` is read from the front, above the account number, while for other networks it is the 3-digit code on the back. `security_code_2` is redefined to mean an ADDITIONAL security code on cards that print more than one; it is `null` on the ordinary card that prints only one code (the usual case, including ordinary Amex cards), and it is never a duplicate of `cvv` (see the normalization rule above).
