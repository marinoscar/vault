...
**Exactly one payload type: a full, uncropped camera-frame photo, as a base64 data URL, sent inside the JSON body of an outbound HTTPS request to OpenAI's API.** `POST /api/secrets/cards/extract` accepts `front` (required) and `back` (optional) in this full-frame form and forwards them to the configured vision model. **This is a privacy-relevant change from earlier behavior, and must be stated plainly: the entire captured frame — including any background around the card (a desk, a hand, other nearby objects or documents) — now leaves the device and reaches OpenAI, not just the card rectangle.** The client (`apps/web`) downscales the photo only when its long edge exceeds 3072px, and otherwise sends it uncropped and unrotated; the model is asked to locate the card itself, and its response carries a per-image bounding box (`crops.front` / `crops.back` — fractional `x`/`y`/`width`/`height`, `quarterTurns`, `confidence`) that the client uses to crop the copy it actually stores. The API itself performs no cropping and never sees or stores either the full frame or the derived crop — see Persistence, below. This full-frame egress does not change what reaches this application's own storage: only the AI-cropped (or user-adjusted) card rectangle is ever saved as a `card_front`/`card_back` attachment, never the surrounding frame.
...
### What Is Sent, Returned, and Stored — Including the Security Code

**The card security code (`cvv`/CVV/CVC) is sent to the model, returned in the response, and stored with the rest of the card, at the repository owner's explicit direction.** This document previously described `cvv`'s exclusion from `EXTRACTED_FIELD_NAMES` as a structural control — no field in the request, no key in the response schema, so no code path could return it even if it were visible in the image. **That control no longer exists and this section must not imply otherwise.** `cvv` is now an ordinary member of `EXTRACTED_FIELD_NAMES` (`apps/api/src/ai/providers/vision-provider.interface.ts`), has a key in `OPENAI_CARD_JSON_SCHEMA`, is asked for and returned exactly like `number`, `exp_month`, or `cardholder_name`, and seeds the review form instead of being forced to an empty string.

**Consequence for the blast radius of a compromise:** the full PAN and the security code now live together in this vault. A compromise that exposes decrypted secret data — a stolen `VAULT_ENCRYPTION_KEY` combined with database access, or an application-level bug that leaks decrypted values — yields a complete card-not-present credential in one place, where previously the CVV's absence meant a stolen record was short one field an attacker would have to source elsewhere. This is the same posture as mainstream consumer password managers, which routinely store both together, and is a deliberate product decision, not an oversight; it is recorded here so it is never mistaken for one.

**Extracting `cvv` adds no new third-party egress.** The photograph sent to OpenAI already contains the security code — it is printed on the physical card being photographed — and that full photograph was already transmitted whether or not the model was asked to transcribe the code back. Requesting `cvv` only decides whether a value already within the provider's view is also returned to this application; it does not cause anything to be sent that wasn't being sent already.

**Once saved, the value is encrypted at rest exactly like every other secret field.** The security code goes through the same `CryptoService` (AES-256-GCM, `VAULT_ENCRYPTION_KEY`) as `number` and every other field on the `Card` secret type — see §6 Database Security Model and `docs/SECRETS.md` § Encryption. Storing it introduces no new encryption gap; it only means the encrypted blob is worth more if that encryption is ever defeated.

**This is a single-owner posture, and it does not survive a change of deployment model.** PCI-DSS forbids retaining the card verification code after authorization (Requirement 3.3.1 in v4.0, 3.2 in v3.2.1), and that requirement binds entities storing, processing, or transmitting cardholder data within the payments ecosystem — not an individual keeping their own cards in their own vault, which is why consumer password managers store the field freely. What separates the two is how this application is deployed, not what the code does: running it as a multi-tenant service, holding an organisation's corporate cards in it, or connecting it to anything that authorises transactions would very plausibly bring it into PCI-DSS scope, at which point `cvv` storage is a compliance violation. Removing the field is a prerequisite for any of those deployments, not a review item. See `docs/SECRETS.md` § Security Considerations.

**`security_code_2` no longer means the Amex front CID.** Before this change, this document (and `docs/SECRETS.md`) described `security_code_2` as the *secondary* code distinct from the CVV, citing the American Express front-printed CID as the example. That CID **is now `cvv` itself**: for American Express, `cvv` is the 4-digit code on the front, above the account number; for other networks it remains the 3-digit code on the back. `security_code_2` is redefined to mean an ADDITIONAL code, present only on cards that print more than one — `null` for the ordinary card (including an ordinary Amex), and the server-side normalizer (`normalizeSecurityCode` in `apps/api/src/ai/card-normalizer.ts`) nulls it outright if the model ever reports it as a duplicate of `cvv`.

**Normalization bounds what a wrong `cvv` reading can become.** `normalizeSecurityCode` strips non-digit characters and accepts the result only at a real length — 3 or 4 digits — nulling anything else (a misread PAN tail, an expiry, a "MEMBER SINCE" year) with confidence reset to `0` and a warning directing the user back to the card. This is a data-quality control, not a security control: it bounds what a wrong reading can look like on the review screen, it does not change what is sent or stored once a genuine code is confirmed.

**Card renewal never carries the outgoing card's `cvv` forward.** `useCardRenewal.ts` (`CVV_FIELD`) seeds every other field from the outgoing card so an unchanged value survives a renewal, but deliberately excepts `cvv`: a reissued card always carries a new code, so the field is seeded only from a fresh reading of the *new* card's photo, falling back to empty rather than silently reusing a value that would produce a card that looks complete and declines.

The full, unmasked card number, expiry, cardholder name, network, kind, issuing bank, `security_code_2`, and `notes` are also sent (as fields the model is asked to fill in), because reading them from the image is the entire point of the feature. `notes` is auxiliary, non-sensitive printed text only — customer service numbers, a "Member Since" year, a website, a contactless indicator, usage instructions — and both the prompt and the response schema explicitly ban the PAN and any security code (`cvv` or `security_code_2`) from ever appearing in it. The one exception to "transcription only" is `kind`: when a card doesn't print "Credit", "Debit", or "Prepaid", the model may classify it from unambiguous product knowledge instead of reading it, fenced by a moderate confidence (around 0.6, not a hard cap) and a mandatory warning (see `docs/SECRETS.md` § Security Considerations) — every other field in this list, `cvv` included, is always a transcription, never an inference. The model's response also carries `crops.front`/`crops.back` — the bounding box it located the card at, plus a confidence in that placement — which is used only client-side to produce the stored crop and is never itself persisted. Treat all of the above the same as any other card-data egress from a security-review standpoint: it is bounded by the controls described here (opt-in flag, admin-only credential, per-user rate limits, encryption at rest once stored), not eliminated.
...
| **Third-Party Data Egress (AI card scanning)** | Opt-in feature flag, admin-only credential, per-user burst + daily budget, encryption at rest once stored | `ai.enabled` default `false`; `cvv` IS requested/returned and encrypted at rest like every other field — see §8's blast-radius note, not an exclusion; burst limiter + audit-row-counted daily quota in `CardExtractService` |
