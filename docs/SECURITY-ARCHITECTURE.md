...
The full, unmasked card number, expiry, cardholder name, network, kind, issuing bank, the *secondary* security code (`security_code_2` — a different value from the CVV, e.g. the 4-digit code on an Amex front), and `notes` ARE sent, because reading them from the image is the entire point of the feature. `notes` is auxiliary, non-sensitive printed text only — customer service numbers, a "Member Since" year, a website, a contactless indicator, usage instructions — and both the prompt and the response schema explicitly ban the PAN and any security code (CVV or `security_code_2`) from ever appearing in it. The one exception to "transcription only" is `kind`: when a card doesn't print "Credit", "Debit", or "Prepaid", the model may classify it from unambiguous product knowledge instead of reading it, fenced by a capped confidence and a mandatory warning (see `docs/SECRETS.md` § Security Considerations) — every other field in this list is always a transcription, never an inference. Treat this the same as any other card-data egress from a security-review standpoint: it is bounded by the controls above, not eliminated.
