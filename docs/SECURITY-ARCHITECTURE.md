...
The full, unmasked card number, expiry, cardholder name, network, kind, issuing bank, and the *secondary* security code (`security_code_2` — a different value from the CVV, e.g. the 4-digit code on an Amex front) ARE sent, because reading them from the image is the entire point of the feature. The one exception is `kind`: when a card doesn't print "Credit", "Debit", or "Prepaid", the model may classify it from unambiguous product knowledge instead of reading it, fenced by a capped confidence and a mandatory warning (see `docs/SECRETS.md` § Security Considerations) — every other field in this list is always a transcription, never an inference. Treat this the same as any other card-data egress from a security-review standpoint: it is bounded by the controls above, not eliminated.
